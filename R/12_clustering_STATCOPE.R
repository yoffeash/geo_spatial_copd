...
preProc <- preProcess(exac_grouped_quarter_wide_stat,method=c("center","scale")) # center and scale data
exac_grouped_quarter_wide_stat_pre <- predict(preProc, exac_grouped_quarter_wide_stat)
exac_grouped_quarter_wide_stat_pre <- as.data.frame(exac_grouped_quarter_wide_stat_pre)

# hierarchical
set.seed(9)
x=as.matrix(exac_grouped_quarter_wide_stat_pre[1:10,4:16])
...
set.seed(9)
y=as.matrix(exac_grouped_quarter_wide_stat[1:10,4:16])
...
hc_DTW <- tsclust(x, type = "fuzzy", k = 3, distance = "sbd", max.iter=9e+99, seed=9 )
plot(hc_DTW)
hc_DTW@cluster
