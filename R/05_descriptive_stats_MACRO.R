...
exac_aov <- aov(Number_Exacerbs~region_cluster_h_1, data=copd_region)
summary(exac_aov)
aggregate(Number_Exacerbs ~ region_cluster_h_1, copd_region, mean)
...
### descriptive statistics and differences in them by regions (using "region_cluster_h_1" variable) ###

## difference in regions based on age ##
age_aov <- aov(age ~ region_cluster_h_1, data=copd_region)
summary(age_aov)

t.test(copd_region$age~copd_region$region_cluster_h_1)

aggregate(age ~ region_cluster_h_1, copd_region, mean)

## difference in region based on GOLD stage ##
region_gold_table <- table(copd_region$region_cluster_h_1, copd_region$goldclass)
region_gold_table
chisq.test(region_gold_table)
CrossTable(copd_region$region_cluster_h_1, copd_region$goldclass)

## difference in region based smoking ##
region_smok_table <- table(copd_region$region_cluster_h_1, copd_region$nowsmk)
region_smok_table
chisq.test(region_smok_table)
CrossTable(copd_region$region_cluster_h_1, copd_region$nowsmk)
...
## difference in prior exacerbation by cluster
region_priorexac_table <- table(copd_region$region_cluster_h_1, copd_region$priorexac)
region_priorexac_table
chisq.test(region_priorexac_table)
CrossTable(copd_region$region_cluster_h_1, copd_region$priorexac)

## difference in oxygen use by cluster
region_oxygen_table <- table(copd_region$region_cluster_h_1, copd_region$oxygen)
region_oxygen_table
chisq.test(region_oxygen_table)
CrossTable(copd_region$region_cluster_h_1, copd_region$oxygen)
